...
const Workshop = ({ date, title, description, result }) => {
  return (
    <div className='w-full flex flex-col items-center justify-start text-dark dark:text-white'>
      ...
      <p className='dark:text-dark-6 text-lg text-body-color place-self-end mt-auto'><span className='underline text-dark-3 dark:text-dark-7'>Результат:  </span>{result}</p>
    </div>
  )
}
