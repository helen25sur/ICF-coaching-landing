...
const Workshop = ({ date, title, description, result }) => {
  return (
    <div className='w-full text-dark dark:text-white'>
      ...
      <p className='dark:text-dark-6 mb-9 text-lg text-body-color'>{description}</p>
      <p className='dark:text-dark-6 mb-9 text-lg text-body-color'><span className='underline text-dark-3 dark:text-dark-7'>Результат:  </span>{result}</p>
    </div>
  )
}
